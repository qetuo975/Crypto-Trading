...
class Adding_Data():
    ...
    # Tüm hepsinin fonksiyonları hazır kanka her verinin altına yazarsın en son eklenenleri vs.
    # Sıkıntı olursa düzeltirim.

    def BUSD_VERI_EKLE(BUSD):
        DATA = {
            "username": username10,
            "BUSD": BUSD,
        }
        cryptodata.insert_one(DATA)
    def TUSD_VERI_EKLE(TUSD):
        DATA = {
            "username": username10,
            "TUSD": TUSD,
        }
        cryptodata.insert_one(DATA)
    def CRYPTO_VARLIKLAR_VERI_EKLE(VARLIK1):
        DATA = {
            "username": username10,
            "CRYPTO_VARLIK": VARLIK1,
        }
        cryptodata.insert_one(DATA)
        # Fazla varlık varsa veya farklı coinleri listeliceksen Fonksiyonda varklıkları arttırıp dict e koyabilirsin knk.
    def ANLIK_SAAT_VERI_EKLE(SAAT):
        DATA = {
            "username": username10,
            "ANLIK SAAT": SAAT,
        }
        cryptodata.insert_one(DATA)
    def KULLANICI_HEDEF_VERI_EKLE(HEDEF):
        DATA = {
            "username": username10,
            "KULLANICI_HEDEF": HEDEF,
        }
        cryptodata.insert_one(DATA)
    def KULLLANICI_TUTAR_VERI_EKLE(TUTAR):
        DATA = {
            "username": username10,
            "TUTAR": TUTAR,
        }
        cryptodata.insert_one(DATA)
    def SON_ODEME_VERI_EKLE(ODEME):
        DATA = {
            "username": username10,
            "ÖDEME": ODEME,
        }
        cryptodata.insert_one(DATA)
    def ANLIK_BITIS_VERI_EKLE(BITIS):
        DATA = {
            "username": username10,
            "ANLIK_BITIS": BITIS,
        }
        cryptodata.insert_one(DATA)
    def ALIM_TARIH_VERI_EKLE(ALIM):
        DATA = {
            "username": username10,
            "ALIM_TARIH": ALIM,
        }
        cryptodata.insert_one(DATA)
    def SATIM_TARIH_VERI_EKLE(SATIM):
        DATA = {
            "username": username10,
            "SATIM_TARIH": SATIM,
        }
        cryptodata.insert_one(DATA)
    def AL_SAT_GECMIS_VERI_EKLE(GECMIS):
        DATA = {
            "username": username10,
            "AL_SAT_GECMIS": GECMIS,
        }
        cryptodata.insert_one(DATA)  # Bunlarda fazla veri olabilir kanka çoğaltabilirsin aynı şekil.
class Intelence():
